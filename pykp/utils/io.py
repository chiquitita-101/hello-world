...
class KeyphraseDataset(torch.utils.data.Dataset):
    def __init__(self, examples, word2idx, idx2word, device, load_train=True,
        ...
        keys = ['src', 'src_oov', 'oov_dict', 'oov_list', 'src_str', 'trg_str', 'trg_gen', 'trg_copy', 'trg_ext']
    ...
    def collate_fn_common(self, batches, trg_gen=None, trg_oov=None, trg_ext=None):
        ...
        if self.load_train:
            seq_pairs = sorted(zip(src_lens, trg_gen, trg_oov, trg_ext), key=lambda p: p[0], reverse=True)
            _, trg_gen, trg_oov, trg_ext = zip(*seq_pairs)
        ...
        if self.load_train:
            if self.fix_kp_num_len:
                #trg_gen batch_size * phrase_num * phrase_length
                trg_gen, trg_lens, trg_mask = self._pad2d(trg_gen)
                trg_oov, _, _ = self._pad2d(trg_oov)
            else:
                trg_gen, trg_lens, trg_mask = self._pad(trg_gen)
                trg_oov, _, _ = self._pad(trg_oov)

            trg_ext, _, _ = self._pad(trg_ext)
            trg_ext = trg_ext.to(self.device)
            trg_gen = trg_gen.to(self.device)
        ...
        return src, src_lens, src_mask, src_oov, oov_lists, src_str, \
               trg_str, trg_gen, trg_oov, trg_lens, trg_mask, trg_ext, original_indices
    ...
    def collate_fn_one2seq(self, batches):
        if self.load_train:
            ...
            for b in batches:
                trg_concat = []
                trg_oov_concat = []
                trg_size = len(b['trg_gen'])
                assert len(b['trg_gen']) == len(b['trg_copy'])
                for trg_idx, (trg_phase, trg_phase_oov) in enumerate(zip(b['trg_gen'], b['trg_copy'])):
                ...
                trg_gen.append(trg_concat)
                trg_oov.append(trg_oov_concat)
            trg_ext = [b['trg_ext'] for b in batches]
            return self.collate_fn_common(batches, trg_gen, trg_oov, trg_ext)
        else:
            return self.collate_fn_common(batches)
    ...
    def collate_fn_fixed_tgt(self, batches):
        if self.load_train:
            if self.seperate_pre_ab:
                ...
                trg_oov = []
                for b in batches:
                    targets = [t for t in b['trg_gen'] if len(t) <= (self.max_kp_len - 1)]
                    ...
                    trg_gen.append(present_targets + extra_present_targets + absent_targets + extra_absent_targets)
                    trg_oov.append(
                ...
                for b in batches:
                    targets = [t + [self.word2idx[EOS_WORD]] + [self.word2idx[PAD_WORD]] * (
                            self.max_kp_len - len(t) - 1)
                               for t in b['trg_gen'] if len(t) <= (self.max_kp_len - 1)][:self.max_kp_num]
                    ...
                    trg_gen.append(targets + extra_targets)
...
def build_extraction_label(source, targets):
    return_label = [2] * len(source) #outside
    source_str = ' '.join(source)
    for target in targets:
        if ' '.join(target) in source_str:#present keyphrase
            #assert source.count(target[0]) > 0
            index = [i for i, word in enumerate(source) if word == target[0]]
            for i in index:
                if (i + len(target)) <= len(source) and ' '.join(source[i : i + len(target)]) == ' '.join(target):
                    return_label[i] = 0
                    for j in range(i + 1, i + len(target)):
                        return_label[j] = 1
    return return_label
def build_dataset(src_trgs_pairs, opt, mode='one2many', include_original=True):
    '''
    ...
    for idx, (source, targets) in enumerate(src_trgs_pairs):
        ...
        trg_ext = build_extraction_label(source, targets)
        ...
        for target in targets:
            ...
            example['trg_ext'] = trg_ext
            if len(oov_list) > max_oov_len:
                max_oov_len = len(oov_list)
                max_oov_sent = source

            trg_gen = [word2idx[w] if w in word2idx and word2idx[w] < opt.vocab_size else word2idx[UNK_WORD]
                   for w in target]
            example['trg_gen'] = trg_gen
        ...
        for key in keys:
            if key.startswith('src') or key.startswith('oov') or key.startswith('trg_ext'):
                o2m_example[key] = examples[0][key]
            else:
                o2m_example[key] = [e[key] for e in examples]
        if include_original:
            assert len(o2m_example['src']) == len(o2m_example['src_oov']) == len(o2m_example['src_str']) == len(o2m_example['trg_ext'])
            assert len(o2m_example['oov_dict']) == len(o2m_example['oov_list'])
            assert len(o2m_example['trg_gen']) == len(o2m_example['trg_copy']) == len(o2m_example['trg_str'])
        else:
            assert len(o2m_example['src']) == len(o2m_example['src_oov']) == len(o2m_example['trg_ext'])
            assert len(o2m_example['oov_dict']) == len(o2m_example['oov_list'])
            assert len(o2m_example['trg_gen']) == len(o2m_example['trg_copy'])
